...
Alucard = commands.Bot(command_prefix=commands.when_mentioned_or(
    ">", "x", "$", "-", "a", "^", "!"), description='Bentley', intents=intents)
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                                ...
                                {Fore.CYAN}VERSION:{Fore.GREEN} v{SELFBOT.__version__}(official)
                                ...
                                {Fore.CYAN}Written by: {Fore.GREEN}JKBentley
                                {Fore.CYAN}Info: {Fore.GREEN}More cracks in our discord server: https://discord.gg/53JF6mtB
    '''+Fore.RESET)
...
[ {ctx.prefix}fun ] - fun and entertainment.
[ {ctx.prefix}reaction ] - interaction teams.
[ {ctx.prefix}mod ] - moderation teams.
[ {ctx.prefix}crash ] - crash teams.
[ {ctx.prefix}text ] - commands for working with text.
[ {ctx.prefix}token ] - commands for working with tokens.
[ {ctx.prefix}tools ] - useful tools.
[ {ctx.prefix}animals ] - teams with animals.
[ {ctx.prefix}status ] - status commands.
[ {ctx.prefix}games ] - different games.
```

**[My server](https://discord.gg/53JF6mtB)**
...
[ {ctx.prefix}vape <Text> ] -  makes spaces between letters caps.
[ {ctx.prefix}animate <Text> ] - makes the message animated.
[ {ctx.prefix}reverse <Text> ] - writes the text in reverse.
[ {ctx.prefix}destroy <number> ] -  explosion of a selected number of messages.
[ {ctx.prefix}spoiler <Text> ] - spoil the whole message.
[ {ctx.prefix}spoiler1 <Text> ] - spoiler each letter separately.
[ {ctx.prefix}spoiler2 <Text> ] - spoilers each letter separately, but does not spoil spaces.
[ {ctx.prefix}aboba <number of messages> ] - changes a certain number of your messages to abobu.
[ {ctx.prefix}s <Text> ] - writes a message on behalf of the bot.
[ {ctx.prefix}edit <number> <Text> ] - edits the entered number of your messages to the entered text.
```
...
[ {ctx.prefix}reacts <number of messages | emoji> ] - puts the selected reaction on a certain number of messages.
[ {ctx.prefix}flex ] - dancing in the chat.
[ {ctx.prefix}fbi [@Ping | ID] - the mentioned participant will be tied by the FSB.
[ {ctx.prefix}gay <@Ping | ID> ] - insult the participant.
[ {ctx.prefix}boom ] - explosive message.
[ {ctx.prefix}fleshka ] - throw a blinding grenade.
[ {ctx.prefix}animatenick <New nick> ] - make yourself an animated nickname.
[ {ctx.prefix}stopanimatenick ] - stop nick animation.
[ {ctx.prefix}thanos ] - click of thanos.
[ {ctx.prefix}joke ] - joke.
[ {ctx.prefix}dick <mentioning> ] - a member of the mentioned user.
[ {ctx.prefix}combine <nick1> <nick2> ] - combines 2 nicknames.
[ {ctx.prefix}say <text> ] - sends text in a frame, replaces some letters with numbers.
[ {ctx.prefix}tweet <name> <text> ] - will create a tweet with the specified author and text.
```
...
[ {ctx.prefix}minesweeper <размер поля> ] - sapper.
[ {ctx.prefix}slot ] - slots.
```
...
[ {ctx.prefix}kiss [@Ping | ID] - поцеловать participant.
[ {ctx.prefix}poke [@Ping | ID] - тыкнуть participant.
[ {ctx.prefix}sex  [@Ping | ID] - заняться с участником сексом.
[ {ctx.prefix}pet  [@Ping | ID] - погладить participant.
[ {ctx.prefix}hit  [@Ping | ID] - ударить participant.
[ {ctx.prefix}kill [@Ping | ID] - убить participant.
```
...
'''
    ...
    embed = discord.Embed(
        title='Commands взаимодействия с participantми.',
...
[ {ctx.prefix}ban <@Ping | ID> ] - забанить participant.
...
[ {ctx.prefix}spamls <@Ping | ID> <text> ] - спам в лс упомянутому участнику.
[ {ctx.prefix}banall ] - забанить всех участников на сервере.
[ {ctx.prefix}kickall ] - кик всех участников на сервере.
[ {ctx.prefix}admineveryone ] - админка всем participantм на сервера.
...
[ {ctx.prefix}emoji_info  [Emoji] - Инфо о emoji.
[ {ctx.prefix}role_info  [@Ping] - Инфо о роли.
[ {ctx.prefix}av <mentioning юзера> ] - получает и отпрвляет аватарку упомянутого юзера.
[ {ctx.prefix}revav <mentioning юзера> ] - исчет автарку упомянутого юзера в гугле.
...
[ {ctx.prefix}encode <text на английском> ] - кодирует введенное сообщение.
[ {ctx.prefix}decode <закодированный text> ] - декодирует введенное сообщение.
...
[ {ctx.prefix}purge <number of messages> ] - удаления определённого количества ваших of messages.
...
async def server_info(ctx):
      ...
      f"•:busts_in_silhouette: ** на сервере** `{ctx.guild.member_count}`\n\n"
            ...
            f"•:keyboard: **Textовых каналов:** `{alltext}`\n\n"
        ...
        embed.set_footer(
            text='BENTLEY© Copyright 2021 | Все права защищены',
...
@Alucard.command(
        aliases=['emoji-info', 'Emoji-info', 'Emoji-инфо', 'emoji-инфо'])
async def emoji_info(ctx, emoji: discord.Emoji = None):
        if emoji is None:
            await ctx.send(embed=discord.Embed(
                description=f':x:**{ctx.author.name}** введите emoji',
                colour=0xffde0a))
        else:
            embed = discord.Embed(
                description=f"[Emoji]({emoji.url}) сервера {emoji}",
                colour=0xffde0a)
            embed.add_field(
                name="Name:",
                value=f"`{emoji.name}`")
...
'''
    ...
    await ctx.send('*Attempt.*', embed=embed)
    for x in range(30):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_sadcat.png"))
    except:
        await ctx.send(link)
...
async def webspam(ctx, webhook=None, count=None, message=None):
    if webhook == None:
        error = f'''
```css
[ {ctx.prefix}webspam<webhook>|<number>|<text>]
```
'''
        embed = discord.Embed(
            description=error,
...
'''
        ...
        if int(count) <= 10000:
            ...
            if _data.status_code == 204:
                text = await ctx.send('отправил `0` of messages')
                for i in range(int(count)):
                    try:
                        if int(count1) <= int(count):
                            ...
                            if _data.status_code == 204:
                                count1 += 1
                                await text.edit(content=f'отправил `{count1}` of messages')
                        else:
...
async def reacts(ctx, count=None, reaction=None):
    ...
    if count == None or reaction == None:
        ...
        embed=discord.Embed(
            title="BENTLEY", description=f"Вы не выбрали number спама или emoji.\n{prefix.strip()}spamreact [number, emoji]", color=randcolor)
...
async def edit(ctx, count=None, *, mesg=None):
    ...
    if count == None or mesg == None:
        randcolor=random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="BENTLEY - Редактирование of messages",
                              description=f"Вы не ввели number of messages или text для замены.\n{prefix.strip()}Редактирование [number] [text]", color=randcolor)
        ...
        randcolor=random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(
            title="BENTLEY", description=f"Редактирование прошло успешно.", color=randcolor)
...
async def aboba(ctx, count=None):
    ...
    if count == None:
        randcolor=random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="BENTLEY - Редактирование of messages",
                              description=f"Вы не ввели number of messages.\n{prefix.strip()}Редактирование [number]", color=randcolor)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_cumslut.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_lesbian.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_anal.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_tits.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_blowjob.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def destroy(ctx, count=None):
    ...
    if count == None:
        randcolor=random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="BENTLEY - Self destruction of messages",
                              description=f"Вы не ввели number of messages.\n{prefix.strip()}Редактирование [number]", color=randcolor)
        await ctx.send(embed=embed)
    else:
        edited=0
        randcolor=random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(
            title="BENTLEY - Self destruction of messages", description=f"Self destruction началось...", color=randcolor)
        msg=await ctx.send(embed=embed)
        async for message in ctx.channel.history():
            if int(edited) < int(count) and message.author == Alucard.user and message != msg and message != ':boom:':
                try:
                    await message.edit(content='Self destruction через 3', embed=None)
                    await asyncio.sleep(1)
                    await message.edit(content='Self destruction через 2', embed=None)
                    await asyncio.sleep(1)
                    await message.edit(content='Self destruction через 1', embed=None)
                    await asyncio.sleep(1)
    ...
    randcolor=random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(
        title="BENTLEY - Self destruction of messages", description=f"Self destruction окончено. Уничтожил {edited} of messages", color=randcolor)
    msg=await ctx.send(embed=embed)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"BENTLEY_pussy.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def on_connect():
    ...
    # ctypes.windll.kernel32.SetConsoleTitleW(
        # f'[BENTLEY Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
